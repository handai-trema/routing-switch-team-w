...
ダイクストラのアルゴリズムでなく、幅優先探索による経路選択について実装しているが、まだ未完成の段階である。  
経路選択のコードは[bfs.rb](https://github.com/handai-trema/routing-switch-team-w/blob/develop/lib/bfs.rb)に記載している。  
以下のコードにより、幅優先探索を行っている。
...
ターミナルで以下のコマンドを実行した。
```
ensyuu2@ensyuu2-VirtualBox:~/routing-switch-team-w$ ./bin/trema send_packets --source host1 --dest host3
ensyuu2@ensyuu2-VirtualBox:~/routing-switch-team-w$ ./bin/trema send_packets --source host3 --dest host1
ensyuu2@ensyuu2-VirtualBox:~/routing-switch-team-w$ ./bin/trema send_packets --source host1 --dest host3

```

コマンドの実行後、以下のように表示され、パスができていることが確認できた。
```
Creating path: 33:33:33:33:33:33 -> 0x5:1 -> 0x5:2 -> 0x1:4 -> 0x1:1 -> 11:11:11:11:11:11
Creating path: 11:11:11:11:11:11 -> 0x1:1 -> 0x1:4 -> 0x5:2 -> 0x5:1 -> 33:33:33:33:33:33
